...
/// MistTerm：远程 PTY 若开启 `ixon`，行末 `XON(0x11)` 可能被 tty 吞掉或干扰流控；仅用于对照测试。
#[cfg(feature = "std")]
fn mistterm_skip_zhex_post_line_xon() -> bool {
    std::env::var("MISTTERM_ZMODEM_SKIP_POST_HEX_XON")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

#[cfg(not(feature = "std"))]
const fn mistterm_skip_zhex_post_line_xon() -> bool {
    false
}
    ...
    P: Write + ?Sized,
{
    ...
    let want_xon = frame != Frame::ZACK && frame != Frame::ZFIN && !mistterm_skip_zhex_post_line_xon();
    if want_xon && port.write_byte(XON)?.is_none() {
        return Ok(None);
    }
    Ok(Some(()))
}
