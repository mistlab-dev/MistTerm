...
/// lrzsz 在首轮 ZFILE 后可能先发 **ZRQINIT** 再发 **ZRINIT**；若 `WaitFilePos` 下 recover 只允许一次，
/// 第二次邀请不会排队，表现为 MistTerm 只写出一轮 Zmodem、随后空转。
#[test]
fn test_sender_wait_file_pos_zrqinit_then_zrinit_both_queue_outgoing() {
    let mut sender = Sender::new().unwrap();
    let pre = sender.drain_outgoing();
    sender.advance_outgoing(pre.len());

    sender.start_file(b"t.bin", 100).unwrap();

    let zrqinit = Header::new(Encoding::ZHEX, Frame::ZRQINIT, &[0; 4]);
    let mut zrqinit_bytes = Vec::new();
    zrqinit.write(&mut zrqinit_bytes).unwrap();

    let c1 = sender.feed_incoming(&zrqinit_bytes).unwrap();
    assert!(c1 > 0, "first ZRQINIT should be consumed");
    let first = sender.drain_outgoing().to_vec();
    assert!(
        !first.is_empty(),
        "WaitReceiverInit: expect ZRINIT+ZFILE after first ZRQINIT"
    );
    sender.advance_outgoing(first.len());

    let c2 = sender.feed_incoming(&zrqinit_bytes).unwrap();
    assert!(c2 > 0, "second ZRQINIT should be consumed");
    if sender.drain_outgoing().is_empty() {
        assert!(
            sender.retry_wait_file_pos_handshake().unwrap(),
            "ZRQINIT re-invite should trigger a recover retry"
        );
    }
    let after_zrqinit = sender.drain_outgoing().to_vec();
    assert!(
        !after_zrqinit.is_empty(),
        "WaitFilePos: ZRQINIT re-invite must queue another ZRINIT+ZFILE"
    );
    sender.advance_outgoing(after_zrqinit.len());

    let zrinit = Header::new(Encoding::ZHEX, Frame::ZRINIT, &[0; 4]);
    let mut zrinit_bytes = Vec::new();
    zrinit.write(&mut zrinit_bytes).unwrap();
    let c3 = sender.feed_incoming(&zrinit_bytes).unwrap();
    assert!(c3 > 0, "ZRINIT should be consumed");
    if sender.drain_outgoing().is_empty() {
        assert!(
            sender.retry_wait_file_pos_handshake().unwrap(),
            "ZRINIT re-invite should trigger a recover retry"
        );
    }
    let after_zrinit = sender.drain_outgoing().to_vec();
    assert!(
        !after_zrinit.is_empty(),
        "WaitFilePos: ZRINIT after ZRQINIT recover must still queue (regression guard)"
    );
}
...
fn test_sender_wait_file_pos_retry_queues_outgoing() {
    let mut sender = Sender::new().unwrap();
    let pre = sender.drain_outgoing().len();
    sender.advance_outgoing(pre);

    sender.start_file(b"zsinit.bin", 16).unwrap();

    // Enter WaitFilePos by accepting receiver init and draining the first ZFILE.
    let zrinit = Header::new(Encoding::ZHEX, Frame::ZRINIT, &[0; 4]);
    let mut zrinit_bytes = Vec::new();
    zrinit.write(&mut zrinit_bytes).unwrap();
    let consumed = sender.feed_incoming(&zrinit_bytes).unwrap();
    assert!(consumed > 0);
    let first_offer = sender.drain_outgoing().len();
    assert!(first_offer > 0);
    sender.advance_outgoing(first_offer);

    // Retry should queue a compatible follow-up offer.
    assert!(sender.retry_wait_file_pos_handshake().unwrap());
    let retry_offer = sender.drain_outgoing().to_vec();
    assert!(
        !retry_offer.is_empty(),
        "retry should queue compatible follow-up frames"
    );
    sender.advance_outgoing(retry_offer.len());
}
