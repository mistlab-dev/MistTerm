...
fn create_connection(session_id: usize) -> Result<Session, String> {
    ...
    let port = 22;
    let username = "ubuntu";
    let private_key_path = Path::new("/Users/tianguangyu/.ssh/id_rsa");
    ...
    // 尝试密钥认证
    if session.userauth_pubkey_file(username, None, private_key_path, None).is_ok() {
        println!("[Session {}] Auth with SSH key", session_id);
    } else {
        // 密码认证
        println!("[Session {}] Auth with password", session_id);
        let password = std::env::var("SSH_PASSWORD").unwrap_or_else(|_| "Tian1234".to_string());
