...
struct MistTermApp {
    ...
    input_buffer: std::collections::HashMap<String, String>,
}
...
impl MistTermApp {
    ...
    fn save_sessions(&self) {
        let path = Self::get_sessions_path();
        let configs: Vec<SessionConfig> = self.sessions
            .iter()
            ...
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    ...
                    SshMessage::Connected => {
                        log::info!("Session connection started");
                    }
                    SshMessage::Error(e) => {
                    ...
                    for (idx, session) in self.sessions.iter().enumerate() {
                        ...
                        ui.horizontal(|ui| {
                            if ui.selectable_label(self.selected_session == Some(idx), 
                                format!("{} - {} ({})", sess.config.name, sess.config.host, status)
                            ).clicked() {
                                select_idx = Some(idx);
                            }
            ...
            if let Some(idx) = self.selected_session {
                if let Some(session) = self.sessions.get(idx) {
                        ...
                        .show(ui, |ui| {
                            egui::ScrollArea::vertical()
                                ...
                                .show(ui, |ui| {
                                    ...
                                    output_text.push_str(&sess.output.current_line);
                    ...
                    ui.horizontal(|ui| {
                        ...
                        let mut input_text = self.get_input_text(&input_key);
                        ...
                        if response.has_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter)) {
                            if !input_text.is_empty() {
                                if let Some(session) = self.sessions.get(idx) {
                                    let sess = session.lock();
                                    if let Some(ref handle) = sess.handle {
                                        let cmd = format!("{}\n", input_text);
                                        let _ = handle.send_input(cmd.as_bytes());
                                    }
                                }
                                self.set_input_text(&input_key, "");
                            }
                        } else {
                            self.set_input_text(&input_key, &input_text);
                        }
                    });
                }
            } else {
        ...
        if self.showing_connect_dialog {
            egui::Window::new("Connect to Server")
                ...
                .show(ctx, |ui| {
                    ...
                    ui.label("Password:");
                    ui.text_edit_singleline(&mut self.new_config.password);
                                ...
                                if let Some(ref mut manager) = self.ssh_manager {
                                    ...
                                    if let Some(ref mut manager) = self.ssh_manager {
                                        match manager.create_session_async(SshConfig {
                                            ...
                                            Ok(session_id) => {
                                                log::info!("Started async connection for session {}", session_id);
                                                // 等待连接完成后再启动 shell
