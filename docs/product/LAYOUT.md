...
**功能/UI 文案：** [`UI-GUIDELINES.md`](./UI-GUIDELINES.md)（部分底栏/右 dock 条目见 §六）  
**历史背景：** UI 改造原型与设计稿见 [`docs/archive/`](../archive/)（`改造后原型.html`、`改造设计规范.md`）

维护布局时 **只改本文件列出的模块**；顶栏以本文件 + `UI-GUIDELINES.md` §一～§三为准。
