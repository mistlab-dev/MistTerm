# MistTerm 布局真源（LAYOUT）

**权威线框：** [`改造后原型.html`](改造后原型.html)  
**实现对照：** [`改造设计规范.md`](改造设计规范.md)  
**功能/UI 文案：** [`界面设计规范.md`](界面设计规范.md)（部分底栏/右 dock 条目见 §六）

维护布局时 **只改本文件列出的模块**；顶栏以本文件 + 《界面设计规范》§一～§三为准。
...
改造或新增右 `SidePanel` 时**必读**。2026-05 命令片段栏踩坑总结。
